...
- **JavaScript描画待機対応**: SPAや動的コンテンツサイトのメタディスクリプション、タイトルタグ、H1要素を確実に読み込み
...
### JavaScript描画待機機能
- **Puppeteer連携**: ブラウザエンジンを使用してJavaScript実行後のページを取得
- **DOMContentLoaded待機**: DOMの完全な読み込みを待機
- **networkidle0待機**: ネットワークアクティビティが完全に停止するまで待機
- **SEO要素待機**: タイトルタグ、メタディスクリプション、H1要素の存在を確認
- **フォールバック機能**: JavaScript描画待機が失敗した場合の静的HTML取得
- **設定可能**: ユーザーがJavaScript描画待機の有効/無効を選択可能
