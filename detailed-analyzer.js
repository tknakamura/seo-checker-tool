...
class DetailedAnalyzer {
  ...
  analyzeHeadingStructure($) {
    ...
    const headingSelectors = [
      'h1, h2, h3, h4, h5, h6',
      'H1, H2, H3, H4, H5, H6',  // 大文字版
      '* h1, * h2, * h3, * h4, * h5, * h6'  // ネストされた見出し
    ];
    
    let foundHeadings = false;
    
    for (const selector of headingSelectors) {
      if (foundHeadings) break;
      ...
      $(selector).each((i, el) => {
        ...
        // 重複チェック
        const existingHeading = headings.find(h => 
          h.level === level && h.text === text && h.position === i + 1
        );
